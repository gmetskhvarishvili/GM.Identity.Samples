...
namespace GM.Identity.Sample.Admin.Gateway.API.Authorization;
...
public sealed class OpaqueTokenAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        // Header-resolved multi-tenancy: normalize the caller-asserted X-Tenant-Id into a trusted
        // "tenant_id" claim, which GM.Gateway's actor forwarding re-emits as X-Tenant-Id (stripping the
        // raw client copy). The backend's ICurrentActor.TenantId then drives the tenant query filter.
